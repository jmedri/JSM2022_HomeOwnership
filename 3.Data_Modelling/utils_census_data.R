process_census_data_1_unzip <- function() {
  file_info_list <- list(
    list(table = "DP05", zip = "DP05.zip", files = c("ACSDP5Y2020.DP05-Data.csv", "ACSDP5Y2020.DP05-Column-Metadata.csv")),
    list(table = "B98001", zip = "B98001.zip", files = c("ACSDT5Y2020.B98001-Data.csv", "ACSDT5Y2020.B98001-Column-Metadata.csv")),
    list(table = "S1501", zip = "S1501.zip", files = c("ACSST5Y2020.S1501-Data.csv", "ACSST5Y2020.S1501-Column-Metadata.csv")),
    list(table = "S1903", zip = "S1903.zip", files = c("ACSST5Y2020.S1903-Data.csv", "ACSST5Y2020.S1903-Column-Metadata.csv")),
    list(table = "S2301", zip = "S2301.zip", files = c("ACSST5Y2020.S2301-Data.csv", "ACSST5Y2020.S2301-Column-Metadata.csv")),
    list(table = "S2501", zip = "S2501.zip", files = c("ACSST5Y2020.S2501-Data.csv", "ACSST5Y2020.S2501-Column-Metadata.csv")),
    list(table = "S2502", zip = "S2502.zip", files = c("ACSST5Y2020.S2502-Data.csv", "ACSST5Y2020.S2502-Column-Metadata.csv")),
    list(table = "S2503", zip = "S2503.zip", files = c("ACSST5Y2020.S2503-Data.csv", "ACSST5Y2020.S2503-Column-Metadata.csv")),
    list(table = "S2506", zip = "S2506.zip", files = c("ACSST5Y2020.S2506-Data.csv", "ACSST5Y2020.S2506-Column-Metadata.csv")),
  ...
  for (file_info in file_info_list) {
    ...
    unzip(
      ...
      exdir = file.path(INPUT_PROCESSED_DIR, "census_model", "1_unzip")
    )
  }
}
...
process_census_data_2_long <- function() {
  ...
  table_data <- (
    purrr::map(
      ...
      function(table_info) {
        ...
        annotation_row <- data[1, , drop = TRUE]
        data <- data[-1, ]
        purrr::map(
          table_info[["columns"]],
          function(col_info) {
  ...
  dplyr::bind_rows() |>
  dplyr::mutate(
    NAME = strsplit(NAME, ", ") |> purrr::map(rlang::set_names, c("County", "State"))
  ) |>
  ...
  save_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "2_long", "data.csv"))

  table_data |>
  purrr::map(purrr::pluck, "column_rename") |>
  dplyr::bind_rows() |>
  save_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "2_long", "column_rename.csv"))
}

process_census_data_3_wide <- function() {
  readr::read_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "2_long", "data.csv")) |>
  tidyr::pivot_wider(
  ...
  save_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "3_wide", "data.csv"))
}
...
check_old_new_data <- function() {
  ...
  if (nrow(data_new) != nrow(data_old)) {
    stop("Unequals rows:", nrow(x), nrow(y))
  }
...
process_census_data_4_model <- function() {
  readr::read_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "3_wide", "data.csv")) |>
  ...
  save_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "4_model", "data.csv"))
}

load_census_data <- function() {
  CENSUS_DATA <<- (
    readr::read_csv(file.path(INPUT_PROCESSED_DIR, "census_model", "4_model", "data.csv")) |>
